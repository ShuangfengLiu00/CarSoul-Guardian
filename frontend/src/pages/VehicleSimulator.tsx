...
import type { Alert, HealthSnapshot, MaintenanceSchedule, Vehicle } from "@/services/types";
import { DEMO_VEHICLE, DEMO_LATEST_HEALTH } from "@/services/demoData";
...
interface Part {
  ...
  leadX: number;
  leadY: number;
  /** alert category 关键字（真实告警 category 多为英文 slug，做子串匹配） */
  alertKeys: string[];
  /** maintenance-schedule category 关键字（真实为中文，做子串匹配） */
  schedKeys: string[];
  /** generic repair tip */
  tips: string[];
}
...
const PARTS: Part[] = [
  ...
  },
  {
    ...
    leadX: 520,
    leadY: 252,
    alertKeys: ["engine", "motor", "powertrain", "动力", "发动机", "常规"],
    schedKeys: ["常规", "发动机", "动力"],
...
const BATTERY_THERMAL_LOOP = {
  is_normal: false,
  steps: [
    { step: "perceive", agent: "perception", title: "状态感知", detail: "电池组温度 48.0℃ 超阈值 40℃，冷却效率降至 62%，SOC 异常下降。", status: "done", data: { battery_temp: 48, cooling: 62 } },
    { step: "understand", agent: "diagnosis", title: "故障诊断", detail: "根因：冷却液循环异常导致热积聚，置信度 88%。", status: "done", data: { root_cause: "冷却液循环异常", confidence: 0.88 } },
    { step: "reason", agent: "risk", title: "风险评估", detail: "热失控风险 HIGH，2 小时内演变为故障概率 72%。", status: "done", data: { level: "high", probability: 0.72, eta_hours: 2 } },
    { step: "tool", agent: "explainer", title: "守护建议", detail: "已远程启动电池主动冷却，建议近期前往服务中心检测冷却系统。", status: "done", data: {} },
    { step: "act", agent: "service", title: "服务执行", detail: "已推送 critical 提醒并记录至车辆数字生命档案。", status: "done", data: {} },
  ],
...
const NORMAL_LOOP = {
  is_normal: true,
  steps: [
    { step: "perceive", agent: "perception", title: "状态感知", detail: "全部传感器读数在阈值内，无异常。", status: "done" },
    { step: "normal_report", agent: "explainer", title: "健康报告", detail: "车辆健康指数 90/100，各子系统运转正常。", status: "done" },
    { step: "act", agent: "service", title: "服务执行", detail: "无守护动作触发，记录本次巡检至数字生命档案。", status: "done" },
  ],
  anomalies: [],
  diagnosis: {},
  risk: {},
...
export default function VehicleSimulator() {
  ...
  // 真实车辆数据（身份 / 健康分 / 告警 / 保养计划）
  const [loading, setLoading] = useState(true);
  const [realVehicle, setRealVehicle] = useState<Vehicle | null>(null);
  const [realHealth, setRealHealth] = useState<HealthSnapshot | null>(null);
  const [realAlerts, setRealAlerts] = useState<Alert[]>([]);
  const [realSchedules, setRealSchedules] = useState<MaintenanceSchedule[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      vehicleService.getLatestHealth(vehicle.id),
      vehicleService.listAlerts(vehicle.id),
      vehicleService.listMaintenanceSchedules(vehicle.id),
    ])
      .then(([v, h, a, s]) => {
        if (!alive) return;
        setRealVehicle(v);
        setRealHealth(h);
        setRealAlerts(a?.items ?? []);
        setRealSchedules(s?.items ?? []);
      })
      .catch(() => {
        /* 失败则回退到 DEMO，保证页面不白屏 */
      })
  ...
  // 真实健康分（空则回退 DEMO，绝不伪造真实车辆状态）
  const health = realHealth ?? DEMO_LATEST_HEALTH;
  const dispVehicle = realVehicle ?? DEMO_VEHICLE;
  const telem = TELEM_BY_PHASE[phase];

  // 真实部件分（不再随演示相位变化，始终展示真实健康分）
  const partScore = (p: Part) => health[p.field] ?? 85;
  ...
  const partAlerts = useMemo<Alert[]>(
    () =>
      selectedPart.alertKeys.length
        ? realAlerts.filter((a) =>
            selectedPart.alertKeys.some((k) => (a.category ?? "").toLowerCase().includes(k.toLowerCase())),
          )
        : [],
    [selectedPart, realAlerts],
  );
  const partSchedules = useMemo<MaintenanceSchedule[]>(
    () =>
      selectedPart.schedKeys.length
        ? realSchedules.filter((s) =>
            selectedPart.schedKeys.some((k) => (s.category ?? "").includes(k)),
          )
        : [],
    [selectedPart, realSchedules],
  );
  ...
  return (
    <div>
      ...
      <Paragraph type="secondary" style={{ marginTop: -2, marginBottom: 14 }}>
        数字孪生车辆剖面图 · 点击车辆部件查看真实健康与检修建议 · 一键仿真守护闭环
      </Paragraph>
          ...
          <Card className="cs-card" styles={{ body: { padding: 12 } }}>
            <div className="cs-sim-stage" style={{ color: "#e2e8f0" }}>
              <Row justify="space-between" align="middle" style={{ marginBottom: 8 }}>
                <Space>
                  ...
                  <Text style={{ color: "#e2e8f0", fontWeight: 600, fontSize: 16 }}>
                    {dispVehicle.brand} {dispVehicle.model}
                  </Text>
                  <Tag color="blue">{dispVehicle.year}</Tag>
                  <Text style={{ color: "#94a3b8", fontSize: 13 }}>
                    {dispVehicle.fuel_type === "electric" ? "纯电" : dispVehicle.fuel_type}
                  </Text>
                ...
                {PARTS.map((p) => {
                  ...
                  return (
                    <g key={`lab-${p.key}`} style={{ pointerEvents: "none" }}>
                      <line x1={p.chipX} y1={p.chipY} x2={p.leadX} y2={p.leadY} stroke={color} strokeWidth="1.2" opacity={active ? 0.9 : 0.35} strokeDasharray="3 3" />
                      ...
                      <g className="cs-sim-chip-text" onClick={() => setSelected(p.key)} style={{ cursor: "pointer", pointerEvents: "all" }}>
                        <rect x={p.chipX - 46} y={p.chipY - 14} width="92" height="28" rx="14" fill={active ? color : "#0f1b2d"} fillOpacity={active ? 0.95 : 0.78} stroke={color} strokeWidth="1.5" />
                        <text x={p.chipX} y={p.chipY - 1} textAnchor="middle" fill={active ? "#fff" : color} fontSize="11" fontWeight="700">{p.name}</text>
                        <text x={p.chipX} y={p.chipY + 10} textAnchor="middle" fill={active ? "#fff" : color} fontSize="10" fontWeight="700">{s} 分</text>
                      </g>
                    </g>
                  );
                })}
              ...
              <div className="cs-sim-legend" style={{ marginTop: 8 }}>
                <span className="cs-sim-legend-item"><span className="cs-sim-legend-dot" style={{ background: "#10b981" }} /> 良好 ≥85</span>
                <span className="cs-sim-legend-item"><span className="cs-sim-legend-dot" style={{ background: "#f59e0b" }} /> 关注 60-84</span>
                <span className="cs-sim-legend-item"><span className="cs-sim-legend-dot" style={{ background: "#ef4444" }} /> 需检修 &lt;60</span>
                <span style={{ color: "#64748b" }}>· 点击部件查看详情</span>
              </div>
            </div>

            {/* 演示遥测（仿真推演用的示意遥测，非真实车端实时流） */}
            <div className="cs-sim-telem" style={{ marginTop: 12 }}>
              {[
                { k: "battery_temp", label: "电池温度(演示)", unit: "℃", warn: telem.battery_temp > 40 },
                { k: "battery_soc", label: "电池 SOC", unit: "%" },
                { k: "cooling_efficiency", label: "冷却效率(演示)", unit: "%", warn: telem.cooling_efficiency < 70 },
        ...
        <Col xs={24} lg={9}>
          ...
          >
            <div style={{ textAlign: "center", marginBottom: 12 }}>
              <Progress type="dashboard" percent={selScore} strokeColor={scoreColor(selScore)} format={(p) => `${p}`} />
              <Text type="secondary">健康评分（真实）</Text>
            </div>

            <Divider style={{ margin: "8px 0" }}>风险告警（真实）</Divider>
            {loading ? (
              <div style={{ textAlign: "center", padding: 12 }}><Spin size="small" /></div>
            ) : partAlerts.length === 0 ? (
              <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<span style={{ color: "#10b981" }}>该系统无活跃告警</span>} />
            ) : (
              partAlerts.map((a) => (
                <div key={a.id} style={{ background: a.level === "critical" ? "#fef2f2" : "#fffbeb", border: `1px solid ${a.level === "critical" ? "#fecaca" : "#fde68a"}`, borderRadius: 10, padding: "10px 12px", marginBottom: 8 }}>
                  <Space style={{ width: "100%", justifyContent: "space-between" }}>
                    <Text strong style={{ fontSize: 14 }}>{a.title}</Text>
                    <Tag color={a.level === "critical" ? "red" : "orange"}>{a.level}</Tag>
                  </Space>
                  <Paragraph style={{ margin: "6px 0 0", fontSize: 13, color: "#6b7280" }}>{a.detail}</Paragraph>
                </div>
              ))
            )}
            ...
            {partSchedules.length > 0 && (
              <>
                <Divider style={{ margin: "12px 0 8px" }}>相关保养计划（真实）</Divider>
                {partSchedules.map((s) => (
                  <div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", fontSize: 13 }}>
                    <span>
                      <Tag color={s.status === "overdue" ? "red" : s.status === "due" ? "orange" : "default"}>{s.status}</Tag>
                      {s.item_name}
                    </span>
                    <Text type="secondary">
      ...
      {/* 守护闭环仿真演示（脚本化示意，非真实车端事件流） */}
      <Card className="cs-card" style={{ marginTop: 16 }} title={<Space><AlertOutlined style={{ color: "#3b82f6" }} />守护闭环仿真 · {phase === "idle" ? "正常巡检" : "电池热失控守护演示"}</Space>}
        extra={phase === "idle" ? (<Button type="primary" size="large" icon={<PlayCircleOutlined />} onClick={startSim}>启动仿真守护</Button>) : (<Button size="large" icon={<ReloadOutlined />} onClick={resetSim}>重置仿真</Button>)}>
        <ClosedLoopTrace loop={loop} />
        {phase === "detect" && (<div style={{ marginTop: 10, color: "#b45309", fontSize: 14 }}><AlertOutlined style={{ marginRight: 6 }} />数字孪生感知到电池温度异常上升，正在向守护引擎上报…</div>)}
        {phase === "recovered" && (<div style={{ marginTop: 10, color: "#10b981", fontSize: 14 }}><CheckCircleOutlined style={{ marginRight: 6 }} />主动冷却已生效，电池温度回落至安全区间，守护闭环完成并写入数字生命档案。</div>)}
      </Card>
    </div>
  );
}
